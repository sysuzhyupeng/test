...
class kindpic extends Component {
	...
	getPhoto() {
		let self = this,
		 	params = {
				...
				'nk': self.props.params.nk == 'all'?'':self.props.params.nk,
			};
	...
	render() {
		let self = this,
			 ...
			 nav_foot_data = [{'name':data.brandName,'url':'/kindpic/'+data.kind_id+'/'+data.new_kind_id}];
		return (
	<div>
		...
		{ data&&self.state.kindpicShow ?
			<section>
				<Header nav={true} module={ data.brandName } hdClass="hd-inner hd-inner-blue" titleRight={
					...
					<SumNav at="kindpic" source="kind" kid={data.kind_id} topic={data.topic.id || ''} />

					<Link to={'/kindyear/'+self.props.params.k } className="year-url">按年份 <i className="arrows-icon"></i></Link>
